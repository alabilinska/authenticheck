...
- [x] (WP-2, blocked by design) upload OK — version `a14ead66-fac2-4664-9488-03aaddc909e7` (tag v0.0.1, bindings SESSION KV + ASSETS), but wrangler printed **no preview URL**; workers.dev hostnames first failed the TLS handshake (certificate for the just-created subdomain still provisioning), minutes later answered 404 — nothing served because the Worker had no deployment and no workers.dev route yet. `deployments status`: "The Worker authenticheck has no deployments." → first production deploy (H3) comes before any preview. Spec: `npm run build && npx wrangler versions upload --message "first upload" --tag v0.0.1 --preview-alias first` → expect `Worker Version ID` + `Version Preview URL` (`https://<id8>-authenticheck.<sub>.workers.dev`) and alias URL. If wrangler reports the Worker does not exist yet, fall back to `npx wrangler deploy --message "first deploy"` (goes live in not-configured mode — acceptable) and continue with Phase 3 checks.
- [x] Superseded: previews only exist after the first deploy; WP-3v confirmed the version preview URL `https://1519092e-authenticheck.alicja-a-bilinska.workers.dev/` → 200. Spec: verify against the preview URL (see Verification). `npx wrangler versions list` shows v0.0.1.

### Phase 3 — Production + rollback rehearsal (human gate: promote)

- [x] **Human** (2026-09-13 00:02): `npm run build && npx wrangler deploy --message "v0.0.1 first deploy"` — chosen over `versions deploy` because only `deploy` applies non-versioned settings/triggers (workers.dev route). Live at `https://authenticheck.alicja-a-bilinska.workers.dev`, Current Version ID `1519092e-44de-46d6-9207-f1e36b8ac295`; bindings SESSION KV (`0cecf801…`) + ASSETS; startup 23 ms; 1910 KiB / 390 KiB gzip. Wrangler warned that `workers_dev` and `preview_urls` were enabled **by default** — pin both explicitly in `wrangler.jsonc` (follow-up below).
- [x] (WP-3v, re-checked by orchestrator) `deployments status` → `1519092e…` at 100 %. 7/7 checks on prod: `/` 200, `/auth/signin` 200 (0.10–0.14 s warm), `/dashboard` 302 → `/auth/signin`, `/does-not-exist` 404, banner present, `/_astro/Layout.C3DlErAY.css` 200; version preview URL `https://1519092e-authenticheck.alicja-a-bilinska.workers.dev/` 200. `tail --status error` 60 s over two traffic rounds: 0 errors. Extra: POST `/api/auth/signin` without an `Origin` header → 403 "Cross-site POST form submissions are forbidden" (Astro `checkOrigin`, expected); with `Origin` → 302 `/auth/signin?error=Supabase%20is%20not%20configured`. Any scripted POST check must send `Origin`. Spec: `npx wrangler deployments status` → version at 100 %. Run the production curls with `npx wrangler tail --format pretty --status error` open in a second terminal — expect silence.
- [x] **Rollback rehearsal** — step 1 done 2026-09-13 00:07: human `npm run build && npx wrangler deploy --message "v0.0.2 rollback rehearsal"` → `6e9f9804-339f-414e-9942-deaaf344b501` at 100 % (verified: `deployments status`, prod `/` 200); no `workers_dev`/`preview_urls` warnings any more, confirming the pin. Local commits pushed (`1069a4d..626b351`); CI run 34721804827 green (checkout, setup-node, npm ci, astro sync, lint, build). Step 2 done 00:09: human `npx wrangler rollback 1519092e-44de-46d6-9207-f1e36b8ac295 -m "rehearsal" -y` → SUCCESS, `1519092e` back at 100 % (verified: `deployments status`, `/` `/auth/signin` 200, `/dashboard` 302). Non-interactive `!` shell: wrangler used the `-m` value and the `-y` fallback for its two prompts — without `-y` the confirm would have no answer. Rehearsal complete; last local deploy — from here production changes only via Workers Builds. Spec: second deploy (`--message "v0.0.2"`, trivial change) → `npx wrangler rollback -m "rehearsal"` picks the previous version, warns that bound resources are not rolled back, asks to confirm → then redeploy head. Never delete `authenticheck-session` while any version references it.
- [x] Follow-up: `"workers_dev": true` and `"preview_urls": true` pinned in `wrangler.jsonc`; generated `dist/server/wrangler.json` carries both, dry-run clean. Ships to production with the Phase 4 trigger push.
- [x] Done 2026-09-13: Supabase project created by the developer (region eu-central-1; Data API on, auto-expose of new tables off, automatic RLS on; email confirmation off for testing — re-enable before external users). Local `.dev.vars` filled in the editor (values never in chat or git); production secrets via `npx wrangler secret bulk .dev.vars` (human) → `SUPABASE_KEY`, `SUPABASE_URL`; new version `7f2e9e37-a7e9-48ae-bc28-7213935673a4` at 100 %. Verified locally and in prod: banner gone, `/auth/signup` `/auth/signin` 200, `/dashboard` 302, bad-credential sign-in → `?error=Invalid login credentials` from Supabase. Consequence of auto-expose off: every migration that creates a table must grant it to `authenticated` and add RLS policies. Originally planned: Later (when Supabase exists): **human** `npx wrangler secret put SUPABASE_URL` / `SUPABASE_KEY` (project in `eu-central-1`); banner disappears without a rebuild; also write both into local `.dev.vars` (not `.env` — wrangler ignores `.env` when `.dev.vars` exists). If `1102` shows in `tail` → Paid plan.
...
- [x] (WP-4v, finished by orchestrator after the subagent stalled on its own poll) Push `626b351..a5fe519` at ~22:15 UTC → new deployment 22:15:49 UTC, version `4480c21e-82e1-43e2-981e-42c5d1477984` at 100 %, no message (Builds does not pass `--message`), compat 2026-05-14 + nodejs_compat. 7/7 prod checks (`/` 200, `/auth/signin` 200 in 0.10 s, `/dashboard` 302 → `/auth/signin`, 404, banner, CSS 200); GitHub Actions on `a5fe519` green; `ci.yml` has 0 `wrangler` mentions. Attribution to Workers Builds is **indirect**: wrangler reports source `wrangler` / `triggered_by: version_upload` and shows no commit or branch; evidence is the missing message (all three manual deploys had one) and the 35–50 s gap after the push. Confirm in the dashboard (Worker → Deployments/Builds) that the build for `a5fe519` produced `4480c21e`. — Confirmed by the developer in the dashboard (2026-09-13). The next push (`667dfeb`) auto-deployed as `17e2b717-c786-4a01-8d5d-ef22ac528144` at 22:24:01 UTC; prod `/` 200, CI green. Wrangler's OAuth token cannot read the Builds API — build logs only in the dashboard. Order changed vs. plan: the push went **before** connecting, because `origin/main` still had the old Worker name. Trigger by pushing the Phase 5 docs commit → Deployments tab shows the build; `npx wrangler deployments list` shows a new deployment with GitHub metadata; production curls pass. From now on production changes only via merge to `main`.
...
- [x] This plan's checkboxes carry the outcome of every WP report and human gate.
...
- `README.md`, `CLAUDE.md`, `context/foundation/infrastructure.md`, `context/deployment/deploy-plan.md` (WP-5a)
