...
| 5   | Etykieta ryzyka na liście „Moje weryfikacje” różni się od werdyktu w zapisanym raporcie, np. po zmianie reguł                    | Medium | Medium     | archiwum `save-and-list-verifications` — wynik liczony przy odczycie, etykieta zapisywana przy zapisie; wywiad P3 (częste zmiany reguł)                                                                     |
...
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- |
| #1   | Każda kombinacja z nieudanym sygnałem twardym daje „wysokie ryzyko”; zmiana lub dodanie reguły nie zmienia werdyktów przypadków z zestawu testowego | „Zestaw testowy przechodzi, więc nie ma fałszywego niskiego” — przypadki są skonstruowane, nie z prawdziwych torebek | Jak sygnały łączą się w poziom ryzyka; zgodność pliku wiedzy z dokumentem reguł co do wartości (tolerancje, okresy), nie tylko identyfikatorów | unit                                       | oczekiwane wartości przepisane z pliku wiedzy (tautologia) — źródłem oczekiwań jest dokument reguł (`balenciaga-city-tag-rules.md` §5) |
| #2   | Błędne lub dwuznaczne dane (literówki, spacje, znaki spoza alfabetu, puste pola) dają błąd wpisu albo wstrzymanie oceny, nigdy sygnał twardy        | „Kreator to wyłapie” — silnik musi być bezpieczny niezależnie od walidacji w interfejsie                             | Które pola mogą dotrzeć do reguł twardych; normalizacja danych; różnica między walidacją kreatora a silnikiem                                  | unit (wiele wariantów literówek)           | testowanie wyłącznie literówek, które już naprawiłyśmy                                                                                 |
...
| --- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------- | ----------- | ------------- | --------------------------------------------- |
| 1   | Silnik reguł bez fałszywych werdyktów | Udowodnić, że podróbka nie dostaje niskiego, a literówka nie daje wysokiego ryzyka; pilnować zgodności etykiety i reguł z dokumentem | #1, #2, #5    | unit        | change opened | context/changes/testing-rule-engine-verdicts/ |
