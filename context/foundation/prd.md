...
### Visual checklist
- FR-006: Buyer can answer three visual checks (black thread on tag, Lampo zipper, spiral hardware twist) as yes / no / can't see, each with a hint and a reference photo; checks whose expected answer depends on era (e.g. zipper variant, changed after 2014) use the year decoded from the tag. Each trait is a soft signal on its own and a hard signal only when the answer contradicts the year decoded from the tag. Priority: must-have
...
The output is a risk level with the signals that set it, plus the missing evidence phrased as copy-ready seller questions. Levels: **high** = at least one hard signal (a rule failure on the tag, or a visual trait whose answer contradicts the year decoded from the tag); **medium** = at least one soft signal (e.g. declared year disagrees with the decoded year, or a visual trait answered "no") or a missing tag photo; **low** = everything checked and consistent. "Can't see" on a visual check does not lower the level but becomes a question.
...
5. **Are the visual checks hard or soft signals?** FR-006 and the Business Logic treat a visual trait answered "no" as a hard signal; the rule knowledge file (§7) makes each trait soft on its own and hard only in combination with the tag year (e.g. a `B` zipper pull on a bag dated 2005). — Owner: user. By: before planning the visual checklist. Block: yes for the visual checklist.
   **Resolved 2026-09-14:** as in the rule knowledge file — each trait is soft on its own and hard only when it contradicts the tag year (FR-006 and Business Logic updated).
