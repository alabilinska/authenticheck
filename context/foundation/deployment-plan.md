...
- [ ] Follow-up: add `"workers_dev": true` and `"preview_urls": true` to `wrangler.jsonc` so the deploy no longer relies on wrangler defaults (ships with the Phase 4 trigger commit).
