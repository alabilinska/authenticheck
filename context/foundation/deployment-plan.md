...
- [ ] **Rollback rehearsal** — step 1 done 2026-09-13 00:07: human `npm run build && npx wrangler deploy --message "v0.0.2 rollback rehearsal"` → `6e9f9804-339f-414e-9942-deaaf344b501` at 100 % (verified: `deployments status`, prod `/` 200); no `workers_dev`/`preview_urls` warnings any more, confirming the pin. Local commits pushed (`1069a4d..626b351`); CI run 34721804827 green (checkout, setup-node, npm ci, astro sync, lint, build). Step 2 (rollback to v0.0.1) pending. Spec: second deploy (`--message "v0.0.2"`, trivial change) → `npx wrangler rollback -m "rehearsal"` picks the previous version, warns that bound resources are not rolled back, asks to confirm → then redeploy head. Never delete `authenticheck-session` while any version references it.
