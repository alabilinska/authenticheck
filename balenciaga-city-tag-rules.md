...
Two other sources agree with the table above, so it stands, but allow a one-season tolerance
when comparing a letter year against a seller's claim.
...
§2 and §3. The criterion is testable as *no invalid string is ever accepted as valid*; it is not
testable as *no counterfeit passes*.
